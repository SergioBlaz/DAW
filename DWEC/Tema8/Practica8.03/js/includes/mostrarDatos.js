...
export const datosListas = (lista) => {
    return `${lista.data().nombre} - ${lista.data().propietario} - ${lista.data().fecha} - [${mostrarArticulosListas(lista.data().articulos)}]`
}
//Función para mostrar los articulos de una lista creada
const mostrarArticulosListas = (obj) =>{
    var articulos
    obj.forEach((e)=>{
        articulos += `Articulo-> Nombre:${e.nombre} Peso:${e.peso}Kg Precio:${e.precio}€`
    })
    return articulos
}
